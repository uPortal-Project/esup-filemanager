...
public class TrustedCmisAccessImpl extends CmisAccessImpl {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(TrustedCmisAccessImpl.class);
    ...
    public void open(SharedUserPortletParameters userParameters) {
        ...
        if(!this.isOpened()) {
            ...
            try {
                ...
                log.warn("failed to retrieve cmisSession '{}', repository is not accessible or simply not started ?", uri, ce);
            }
        }
    }
}
