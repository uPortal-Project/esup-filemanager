...
public class GroupEvaluator implements IDriveAccessEvaluator, InitializingBean {
    ...
    /** The logger. */
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(GroupEvaluator.class);
    ...
    public boolean isApplicable(PortletRequest request) {
        ...
        if (log.isDebugEnabled())
            log.debug(" >>>> calling GroupEvaluator[{}, op={}].isApplicable()", this, type);
        ...
        if (log.isDebugEnabled())
            log.debug(" ---- GroupEvaluator[{}, op={}].isApplicable()= {}", this, type, rslt);
